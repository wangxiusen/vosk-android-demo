# TTS PCM 响度一致化实现与真机验收

日期：2026-08-26

状态：已实现、已通过 JVM 测试、Lint、APK 构建和 Xiaomi 2201122C 真机自动化验收。
...
第四个 Demo 已增加“口播音量”选择，不改变系统媒体音量：
...
目标值使用稳定的 `TtsVolume` 内部枚举，UI 文案和算法参数分离。
...
已新增纯 Java `PcmLoudnessNormalizer`，只负责统计和增益计算；`OfflineTtsPlayer` 仍负责模型、队列和 `AudioTrack` 生命周期。
...
真机：Xiaomi 2201122C，Android 15，arm64-v8a。默认使用“增强音量”，目标有效 RMS 为 `-18 dBFS`。

- 10 个中文女声使用同一文字实际合成和播放，主语音块的输入有效 RMS 为 `-34.82～-25.66 dBFS`，处理后全部为 `-18.00 dBFS`，离散范围 `0.00 dB`。
- 10 个中文主语音块的最高输出峰值为 `-3.27 dBFS`；英文后续块触发峰值保护时为 `-1.00 dBFS`，没有超过限制。
- 后续 PCM 块的队列等待为 `0～1 ms`，标点停顿、中英文切换、播放中停止和原始 PCM 嘴型路径通过回归。
- 冷启动后第一个中文块的响度扫描曾用时 `64 ms`；后续中文主块为 `2～13 ms`，英文首块为 `4～20 ms`。因此“冷启动新增耗时不超过 30 ms”的严格目标未达到；未为追数据增加额外预热等待。
- `testDebugUnitTest`、`lintDebug`、`assembleDebug` 和 `connectedDebugAndroidTest` 全部通过，真机仪器测试 `4/4` 通过。

自动化已证明数字增益、峰值保护和播放链路生效；本轮没有代替用户在目标外置喇叭上做主观听感验收。精简的原始统计日志见 [2026-08-26-tts-pcm-loudness.log](2026-08-26-tts-pcm-loudness.log)。
