...
- Demo 仅发出事件，不控制真实设备。
- 仅修改 `/Users/ws/project/零碎项目/语音识别 android`；目录不是 Git 仓库，不执行提交步骤。
...
- [ ] **Step 3: 配置 Manifest 与主题**

  仅申请 `RECORD_AUDIO`；Activity 非导出；应用标签为 `潓美医疗语音 Demo`。
